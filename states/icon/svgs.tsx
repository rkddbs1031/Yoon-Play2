import { FC, SVGProps } from 'react';

export interface IconProps extends SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
}

export const SearchIcon: FC<IconProps> = ({ size = 24, color = 'currentColor', ...props }) => (
  <svg
    width={size}
    height={size}
    viewBox='0 0 24 24'
    fill='none'
    stroke={color}
    strokeWidth='2'
    strokeLinecap='round'
    strokeLinejoin='round'
    {...props}
  >
    <circle cx='11' cy='11' r='8' />
    <line x1='21' y1='21' x2='16.65' y2='16.65' />
  </svg>
);

export const PlayIcon: FC<IconProps> = ({ size = 20, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 20 20' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M16.055 11.2635C16.8816 10.6239 16.8816 9.37607 16.055 8.7365C13.5571 6.80367 10.7677 5.28021 7.79146 4.22314L7.24773 4.03003C6.20751 3.66058 5.10875 4.36438 4.96792 5.43817C4.57445 8.43835 4.57445 11.5617 4.96792 14.5618C5.10874 15.6356 6.2075 16.3394 7.24773 15.97L7.79146 15.7769C10.7677 14.7198 13.5571 13.1963 16.055 11.2635Z'
        fill={color}
      />
    </svg>
  );
};

export const PrevIcon: FC<IconProps> = ({ size = 20, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 20 20' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M1.2884 10.879C0.713396 10.434 0.713396 9.56594 1.2884 9.12102C3.07017 7.74236 5.05976 6.65569 7.1827 5.9017L7.57108 5.76376C8.29578 5.50638 9.06186 5.9967 9.16005 6.7454C9.27482 7.62057 9.34267 8.51044 9.36359 9.40299C9.43033 9.29827 9.51638 9.20254 9.62174 9.12102C11.4035 7.74236 13.3931 6.65569 15.516 5.9017L15.9044 5.76376C16.6291 5.50638 17.3952 5.9967 17.4934 6.7454C17.7741 8.88582 17.7741 11.1142 17.4934 13.2546C17.3952 14.0033 16.6291 14.4936 15.9044 14.2362L15.516 14.0983C13.3931 13.3443 11.4035 12.2576 9.62174 10.879C9.51638 10.7974 9.43033 10.7017 9.36359 10.597C9.34267 11.4895 9.27482 12.3794 9.16005 13.2546C9.06186 14.0033 8.29578 14.4936 7.57108 14.2362L7.1827 14.0983C5.05976 13.3443 3.07017 12.2576 1.2884 10.879Z'
        fill={color}
      />
    </svg>
  );
};

export const NextIcon: FC<IconProps> = ({ size = 20, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 20 20' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M10.6363 9.40299C10.5696 9.29826 10.4836 9.20254 10.3782 9.12102C8.59643 7.74236 6.60684 6.65569 4.4839 5.9017L4.09552 5.76376C3.37082 5.50638 2.60474 5.9967 2.50655 6.7454C2.22584 8.88582 2.22584 11.1142 2.50655 13.2546C2.60474 14.0033 3.37082 14.4936 4.09552 14.2362L4.4839 14.0983C6.60684 13.3443 8.59643 12.2576 10.3782 10.879C10.4836 10.7974 10.5696 10.7017 10.6363 10.597C10.6573 11.4895 10.7251 12.3794 10.8399 13.2546C10.9381 14.0033 11.7042 14.4936 12.4289 14.2362L12.8172 14.0983C14.9402 13.3443 16.9298 12.2576 18.7115 10.879C19.2865 10.434 19.2865 9.56594 18.7115 9.12102C16.9298 7.74236 14.9402 6.65569 12.8172 5.9017L12.4289 5.76376C11.7042 5.50638 10.9381 5.9967 10.8399 6.7454C10.7251 7.62057 10.6573 8.51044 10.6363 9.40299Z'
        fill={color}
      />
    </svg>
  );
};

export const PauseIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M17.2757 5.46995C17.7109 5.63029 18 6.04492 18 6.50872L18 17.4912C18 17.955 17.7109 18.3696 17.2757 18.53C16.4523 18.8333 15.5477 18.8333 14.7243 18.53C14.2891 18.3696 14 17.955 14 17.4912L14 6.50872C14 6.04492 14.2891 5.63029 14.7243 5.46995C15.5477 5.16659 16.4523 5.16659 17.2757 5.46995Z'
        fill={color}
      />
      <path
        d='M9.27568 5.46995C9.71088 5.63029 10 6.04492 10 6.50872L10 17.4912C10 17.955 9.71088 18.3696 9.27568 18.53C8.45228 18.8333 7.54772 18.8333 6.72432 18.53C6.28912 18.3696 6 17.955 6 17.4912L6 6.50872C6 6.04492 6.28912 5.63029 6.72432 5.46995C7.54772 5.16659 8.45228 5.16659 9.27568 5.46995Z'
        fill={color}
      />
    </svg>
  );
};

export const VolumeIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill={color} xmlns='http://www.w3.org/2000/svg'>
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M11 5L6 9H2V15H6L11 19V5V5Z'
        strokeWidth='2'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
      <path
        d='M19.7772 4.22299C19.3867 3.83241 18.7536 3.83231 18.363 4.22278C17.9724 4.61324 17.9723 5.24641 18.3628 5.63699L19.7772 4.22299ZM18.3628 18.363C17.9723 18.7536 17.9724 19.3867 18.363 19.7772C18.7536 20.1677 19.3867 20.1676 19.7772 19.777L18.3628 18.363ZM16.2472 7.75299C15.8567 7.36241 15.2236 7.36231 14.833 7.75278C14.4424 8.14324 14.4423 8.77641 14.8328 9.16699L16.2472 7.75299ZM14.8328 14.823C14.4423 15.2136 14.4424 15.8467 14.833 16.2372C15.2236 16.6277 15.8567 16.6276 16.2472 16.237L14.8328 14.823ZM18.3628 5.63699C21.8762 9.15149 21.8762 14.8485 18.3628 18.363L19.7772 19.777C24.0714 15.4815 24.0714 8.51849 19.7772 4.22299L18.3628 5.63699ZM14.8328 9.16699C16.3943 10.729 16.3943 13.261 14.8328 14.823L16.2472 16.237C18.5895 13.894 18.5895 10.096 16.2472 7.75299L14.8328 9.16699Z'
        fill={color}
      />
    </svg>
  );
};

export const MutedVolumeIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M10 5L5 9H1V15H5L10 19V5V5Z'
        stroke={color}
        strokeWidth='2'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
      <path d='M22 9L16 15' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
      <path d='M16 9L22 15' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
    </svg>
  );
};

export const DownIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path d='M6 9L12 15L18 9' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
    </svg>
  );
};

export const MoreVerticalIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M12 14C13.1046 14 14 13.1046 14 12C14 10.8954 13.1046 10 12 10C10.8954 10 10 10.8954 10 12C10 13.1046 10.8954 14 12 14Z'
        stroke={color}
        strokeWidth='2'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M12 6C13.1046 6 14 5.10457 14 4C14 2.89543 13.1046 2 12 2C10.8954 2 10 2.89543 10 4C10 5.10457 10.8954 6 12 6Z'
        stroke={color}
        strokeWidth='2'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M12 22C13.1046 22 14 21.1046 14 20C14 18.8954 13.1046 18 12 18C10.8954 18 10 18.8954 10 20C10 21.1046 10.8954 22 12 22Z'
        stroke={color}
        strokeWidth='2'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
    </svg>
  );
};

export const YoonLogoIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M12 3.75C8.43559 3.75 5.75 6.2048 5.75 9V10.5123C5.83223 10.5042 5.91563 10.5 6 10.5H8C8.82843 10.5 9.5 11.1716 9.5 12V17C9.5 17.8284 8.82843 18.5 8 18.5H6C4.61929 18.5 3.5 17.3807 3.5 16V13C3.5 12.3007 3.78716 11.6684 4.25 11.2146V9C4.25 5.16778 7.83242 2.25 12 2.25C16.1676 2.25 19.75 5.16778 19.75 9V11.2146C20.2128 11.6684 20.5 12.3007 20.5 13V16C20.5 17.3807 19.3807 18.5 18 18.5H16C15.1716 18.5 14.5 17.8284 14.5 17V12C14.5 11.1716 15.1716 10.5 16 10.5H18C18.0844 10.5 18.1678 10.5042 18.25 10.5123V9C18.25 6.2048 15.5644 3.75 12 3.75Z'
        fill={color}
      />
    </svg>
  );
};

export const ExploreIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M14.3851 15.4458C11.7348 17.5685 7.85538 17.4014 5.3986 14.9446C2.76256 12.3086 2.76256 8.0347 5.3986 5.39866C8.03464 2.76262 12.3085 2.76262 14.9445 5.39866C17.4013 7.85544 17.5684 11.7349 15.4457 14.3851L20.6014 19.5408C20.8943 19.8337 20.8943 20.3085 20.6014 20.6014C20.3085 20.8943 19.8336 20.8943 19.5407 20.6014L14.3851 15.4458ZM6.45926 13.8839C4.40901 11.8337 4.40901 8.50957 6.45926 6.45932C8.50951 4.40907 11.8336 4.40907 13.8839 6.45932C15.9326 8.50807 15.9341 11.8288 13.8884 13.8794C13.8869 13.8809 13.8854 13.8824 13.8839 13.8839C13.8824 13.8854 13.8809 13.8869 13.8794 13.8884C11.8288 15.9342 8.50801 15.9327 6.45926 13.8839Z'
        fill={color}
      />
    </svg>
  );
};

export const Libraryicon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M19 21L12 16L5 21V5C5 3.89543 5.89543 3 7 3H17C18.1046 3 19 3.89543 19 5V21Z'
        stroke={color}
        strokeWidth='2'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
    </svg>
  );
};

export const HomeIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M9 17H10C10 16.4477 9.55228 16 9 16V17ZM21 15H22C22 14.4477 21.5523 14 21 14V15ZM9 16H5V18H9V16ZM5 16C3.34315 16 2 17.3431 2 19H4C4 18.4477 4.44772 18 5 18V16ZM2 19C2 20.6569 3.34315 22 5 22V20C4.44772 20 4 19.5523 4 19H2ZM5 22H7V20H5V22ZM7 22C8.65685 22 10 20.6569 10 19H8C8 19.5523 7.55228 20 7 20V22ZM10 19V17H8V19H10ZM21 14H17V16H21V14ZM17 14C15.3431 14 14 15.3431 14 17H16C16 16.4477 16.4477 16 17 16V14ZM14 17C14 18.6569 15.3431 20 17 20V18C16.4477 18 16 17.5523 16 17H14ZM17 20H19V18H17V20ZM19 20C20.6569 20 22 18.6569 22 17H20C20 17.5523 19.5523 18 19 18V20ZM22 17V15H20V17H22Z'
        fill={color}
      />
      <path d='M9 17V5L21 3V15' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
    </svg>
  );
};

export const HamburgerIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path d='M3 12H21' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
      <path d='M3 6H21' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
      <path d='M3 18H21' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
    </svg>
  );
};

export const CloseIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path d='M18 6L6 18' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
      <path d='M6 6L18 18' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
    </svg>
  );
};

export const LikeIcon: FC<IconProps> = ({ size = 24, color = 'currentColor', fill = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill={fill} xmlns='http://www.w3.org/2000/svg'>
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M20.84 4.61C19.8085 3.57799 18.4091 2.99817 16.95 2.99817C15.4909 2.99817 14.0915 3.57799 13.06 4.61L12 5.67L10.94 4.61C8.79161 2.46161 5.30838 2.46161 3.16 4.61C1.01161 6.75839 1.01161 10.2416 3.16 12.39L4.22 13.45L12 21.23L19.78 13.45L20.84 12.39C21.872 11.3585 22.4518 9.95914 22.4518 8.5C22.4518 7.04086 21.872 5.64152 20.84 4.61Z'
        stroke={color}
        strokeWidth='2'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
    </svg>
  );
};

export const AddPlaylistIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z'
        stroke={color}
        strokeWidth='2'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
      <path d='M12 8V16' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
      <path d='M8 12H16' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
    </svg>
  );
};

export const MoreHorizontal: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M12 14C13.1046 14 14 13.1046 14 12C14 10.8954 13.1046 10 12 10C10.8954 10 10 10.8954 10 12C10 13.1046 10.8954 14 12 14Z'
        stroke={color}
        strokeWidth='2'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M20 14C21.1046 14 22 13.1046 22 12C22 10.8954 21.1046 10 20 10C18.8954 10 18 10.8954 18 12C18 13.1046 18.8954 14 20 14Z'
        stroke={color}
        strokeWidth='2'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M4 14C5.10457 14 6 13.1046 6 12C6 10.8954 5.10457 10 4 10C2.89543 10 2 10.8954 2 12C2 13.1046 2.89543 14 4 14Z'
        stroke={color}
        strokeWidth='2'
        strokeLinecap='round'
        strokeLinejoin='round'
      />
    </svg>
  );
};

export const TrashIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path d='M3 6H5H21' stroke={color} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
      <path
        d='M20 6C20 5.44772 19.5523 5 19 5C18.4477 5 18 5.44772 18 6H20ZM6 6C6 5.44772 5.55228 5 5 5C4.44772 5 4 5.44772 4 6H6ZM7 6C7 6.55228 7.44772 7 8 7C8.55228 7 9 6.55228 9 6H7ZM15 6C15 6.55228 15.4477 7 16 7C16.5523 7 17 6.55228 17 6H15ZM18 6V20H20V6H18ZM18 20C18 20.5523 17.5523 21 17 21V23C18.6569 23 20 21.6569 20 20H18ZM17 21H7V23H17V21ZM7 21C6.44772 21 6 20.5523 6 20H4C4 21.6569 5.34315 23 7 23V21ZM6 20V6H4V20H6ZM9 6V4H7V6H9ZM9 4C9 3.44772 9.44772 3 10 3V1C8.34315 1 7 2.34315 7 4H9ZM10 3H14V1H10V3ZM14 3C14.5523 3 15 3.44772 15 4H17C17 2.34315 15.6569 1 14 1V3ZM15 4V6H17V4H15Z'
        fill={color}
      />
    </svg>
  );
};

export const PlusIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M12.75 7C12.75 6.58579 12.4142 6.25 12 6.25C11.5858 6.25 11.25 6.58579 11.25 7V11.25H7C6.58579 11.25 6.25 11.5858 6.25 12C6.25 12.4142 6.58579 12.75 7 12.75H11.25V17C11.25 17.4142 11.5858 17.75 12 17.75C12.4142 17.75 12.75 17.4142 12.75 17V12.75H17C17.4142 12.75 17.75 12.4142 17.75 12C17.75 11.5858 17.4142 11.25 17 11.25H12.75V7Z'
        fill={color}
      />
    </svg>
  );
};

export const SpinIcon: FC<IconProps> = ({ color = 'currentColor' }) => {
  return (
    <svg
      className='mr-3 -ml-1 size-5 animate-spin text-white'
      xmlns='http://www.w3.org/2000/svg'
      fill='none'
      viewBox='0 0 24 24'
    >
      <circle className='opacity-25' cx='12' cy='12' r='10' stroke={color} strokeWidth='4' />
      <path
        className='opacity-75'
        fill={color}
        d='M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z'
      />
    </svg>
  );
};

export const EditIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M21.4549 5.41575C21.6471 5.70687 21.615 6.10248 21.3588 6.35876L12.1664 15.5511C12.0721 15.6454 11.9545 15.7128 11.8256 15.7465L7.99716 16.7465C7.87229 16.7791 7.74358 16.7784 7.62265 16.7476C7.49408 16.7149 7.37431 16.6482 7.27729 16.5511C7.08902 16.3629 7.01468 16.0889 7.08197 15.8313L8.08197 12.0028C8.11144 11.89 8.16673 11.7786 8.24322 11.6912L17.4697 2.46967C17.5504 2.38891 17.6477 2.32846 17.7536 2.29163C17.8321 2.26432 17.9153 2.25 18 2.25C18.1989 2.25 18.3897 2.32902 18.5303 2.46967L21.3588 5.2981C21.3954 5.33471 21.4274 5.37416 21.4549 5.41575ZM19.7678 5.82843L18 4.06066L9.48184 12.5788L8.85685 14.9716L11.2496 14.3466L19.7678 5.82843Z'
        fill={color}
      />
      <path
        d='M19.6414 17.1603C19.9148 14.8227 20.0018 12.4688 19.9023 10.1208C19.8976 10.0084 19.9399 9.89898 20.0194 9.81942L21.0027 8.83609C21.1236 8.71519 21.3302 8.79194 21.3415 8.96254C21.5265 11.7522 21.4564 14.5545 21.1312 17.3346C20.8946 19.3571 19.2703 20.9421 17.2583 21.167C13.7917 21.5544 10.2083 21.5544 6.74177 21.167C4.72971 20.9421 3.10538 19.3571 2.86883 17.3346C2.45429 13.7903 2.45429 10.2097 2.86883 6.66543C3.10538 4.6429 4.72971 3.05789 6.74177 2.83301C9.37152 2.5391 12.0685 2.46815 14.7306 2.62016C14.9022 2.62996 14.9804 2.83757 14.8589 2.95909L13.8664 3.95165C13.7877 4.03034 13.6798 4.07261 13.5685 4.06885C11.3421 3.99376 9.10055 4.07872 6.90838 4.32373C5.57827 4.47239 4.51278 5.522 4.35867 6.83968C3.95767 10.2682 3.95767 13.7318 4.35867 17.1603C4.51278 18.478 5.57827 19.5276 6.90838 19.6763C10.2642 20.0513 13.7358 20.0513 17.0916 19.6763C18.4218 19.5276 19.4872 18.478 19.6414 17.1603Z'
        fill={color}
      />
    </svg>
  );
};

export const ListMusic: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M8.75003 13.5C8.75003 13.0858 9.08582 12.75 9.50003 12.75H11.25V11C11.25 10.5858 11.5858 10.25 12 10.25C12.4142 10.25 12.75 10.5858 12.75 11V12.75H14.5C14.9142 12.75 15.25 13.0858 15.25 13.5C15.25 13.9142 14.9142 14.25 14.5 14.25H12.75V16C12.75 16.4142 12.4142 16.75 12 16.75C11.5858 16.75 11.25 16.4142 11.25 16V14.25H9.50003C9.08582 14.25 8.75003 13.9142 8.75003 13.5Z'
        fill={color}
      />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M21.1613 9.74679C21.5581 12.233 21.5297 14.7686 21.0772 17.2453C20.8748 18.353 19.9484 19.1833 18.8253 19.2636L17.1874 19.3808C13.7336 19.6279 10.2665 19.6279 6.81264 19.3808L5.29864 19.2725C4.10261 19.1869 3.1161 18.3027 2.90058 17.1232C2.46392 14.7333 2.38916 12.2913 2.67882 9.87915L2.9513 7.61004C3.11301 6.26343 4.25541 5.25 5.6117 5.25H7.90327C9.02102 5.25 9.95229 6.04846 10.1578 7.10612L18.4722 7.10612C19.7786 7.10612 20.8913 8.05533 21.0972 9.34535L21.1613 9.74679ZM19.6016 16.9757C20.0236 14.6662 20.0501 12.3017 19.6801 9.98322L19.616 9.58178C19.5263 9.0197 19.0414 8.60612 18.4722 8.60612H9.75939C9.17432 8.60612 8.70003 8.13183 8.70003 7.54677C8.70003 7.10672 8.34331 6.75 7.90327 6.75H5.6117C5.01467 6.75 4.51179 7.19611 4.4406 7.78888L4.16812 10.058C3.89639 12.3208 3.96652 14.6116 4.37615 16.8536C4.46868 17.36 4.8922 17.7396 5.40568 17.7763L6.91968 17.8846C10.3023 18.1266 13.6978 18.1266 17.0804 17.8846L18.7183 17.7674C19.1588 17.7359 19.5222 17.4102 19.6016 16.9757Z'
        fill={color}
      />
    </svg>
  );
};

export const ToastSuccuessIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M7.48334 5.25942C6.33891 5.38732 5.42286 6.29057 5.29045 7.42268C4.93476 10.4638 4.93476 13.5361 5.29045 16.5772C5.42286 17.7093 6.33891 18.6126 7.48334 18.7405C10.4602 19.0732 13.5398 19.0732 16.5166 18.7405C17.6611 18.6126 18.5771 17.7093 18.7095 16.5772C18.9651 14.3921 19.037 12.1909 18.9253 9.99668C18.9224 9.94002 18.9436 9.88475 18.9837 9.84463L20.0225 8.80585C20.1427 8.68562 20.3482 8.7608 20.3609 8.93036C20.557 11.5353 20.5031 14.1543 20.1994 16.7515C19.9845 18.5884 18.5096 20.0271 16.6832 20.2312C13.5957 20.5763 10.4043 20.5763 7.31673 20.2312C5.49035 20.0271 4.01545 18.5884 3.8006 16.7515C3.43137 13.5945 3.43137 10.4053 3.8006 7.24843C4.01545 5.41146 5.49035 3.97282 7.31673 3.7687C10.4043 3.42362 13.5957 3.42362 16.6832 3.7687C17.3265 3.84059 17.9261 4.06562 18.4425 4.40725C18.5441 4.47448 18.5542 4.61732 18.468 4.70346L17.6652 5.50635C17.5995 5.57202 17.4976 5.58307 17.4158 5.5392C17.1423 5.39271 16.8385 5.29539 16.5166 5.25942C13.5398 4.92671 10.4602 4.92671 7.48334 5.25942Z'
        fill={color}
      />
      <path
        d='M21.0303 6.03028C21.3232 5.73738 21.3232 5.26251 21.0303 4.96962C20.7374 4.67672 20.2625 4.67672 19.9696 4.96962L11.5 13.4393L9.0303 10.9696C8.73741 10.6767 8.26253 10.6767 7.96964 10.9696C7.67675 11.2625 7.67675 11.7374 7.96964 12.0303L10.9696 15.0303C11.2625 15.3232 11.7374 15.3232 12.0303 15.0303L21.0303 6.03028Z'
        fill={color}
      />
    </svg>
  );
};

export const ToastErrorIcon: FC<IconProps> = ({ size = 18, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 18 18' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M8.75 3.75C9.16421 3.75 9.5 4.08579 9.5 4.5V9.5C9.5 9.91421 9.16421 10.25 8.75 10.25C8.33579 10.25 8 9.91421 8 9.5V4.5C8 4.08579 8.33579 3.75 8.75 3.75Z'
        fill={color}
      />
      <path
        d='M8.75 13.75C9.30228 13.75 9.75 13.3023 9.75 12.75C9.75 12.1977 9.30228 11.75 8.75 11.75C8.19772 11.75 7.75 12.1977 7.75 12.75C7.75 13.3023 8.19772 13.75 8.75 13.75Z'
        fill={color}
      />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M0 8.75C0 3.91751 3.91751 0 8.75 0C13.5825 0 17.5 3.91751 17.5 8.75C17.5 13.5825 13.5825 17.5 8.75 17.5C3.91751 17.5 0 13.5825 0 8.75ZM8.75 1.5C4.74594 1.5 1.5 4.74594 1.5 8.75C1.5 12.7541 4.74594 16 8.75 16C12.7541 16 16 12.7541 16 8.75C16 4.74594 12.7541 1.5 8.75 1.5Z'
        fill={color}
      />
    </svg>
  );
};

export const ToastWarningIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M13.0001 17C13.0001 17.5523 12.5523 18 12.0001 18C11.4478 18 11.0001 17.5523 11.0001 17C11.0001 16.4478 11.4478 16 12.0001 16C12.5523 16 13.0001 16.4478 13.0001 17Z'
        fill={color}
      />
      <path
        d='M12.0001 7.75005C12.4143 7.75005 12.7501 8.08584 12.7501 8.50005V13.5C12.7501 13.9143 12.4143 14.25 12.0001 14.25C11.5858 14.25 11.2501 13.9143 11.2501 13.5V8.50005C11.2501 8.08584 11.5858 7.75005 12.0001 7.75005Z'
        fill={color}
      />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M14.2704 3.99268C13.1784 2.39502 10.8216 2.39501 9.72967 3.99268L9.29775 4.62463C6.59171 8.58384 4.26675 12.7905 2.35402 17.1881L2.2638 17.3956C1.58975 18.9453 2.60811 20.705 4.28766 20.8927C9.41329 21.4656 14.5868 21.4656 19.7124 20.8927C21.392 20.705 22.4103 18.9453 21.7363 17.3956L21.6461 17.1881C19.7333 12.7905 17.4084 8.58385 14.7023 4.62463L14.2704 3.99268ZM10.9681 4.83909C11.4644 4.11288 12.5357 4.11288 13.032 4.83909L13.4639 5.47104C16.1165 9.35206 18.3956 13.4756 20.2705 17.7864L20.3608 17.9938C20.6322 18.6179 20.2221 19.3264 19.5458 19.402C14.5309 19.9625 9.46917 19.9625 4.45427 19.402C3.77797 19.3264 3.36791 18.6179 3.63933 17.9938L3.72954 17.7864C5.6045 13.4756 7.88354 9.35205 10.5361 5.47104L10.9681 4.83909Z'
        fill={color}
      />
    </svg>
  );
};

export const ToastInfoIcon: FC<IconProps> = ({ size = 24, color = 'currentColor' }) => {
  return (
    <svg width={size} height={size} viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
      <path
        d='M12 10.7499C12.4142 10.7499 12.75 11.0857 12.75 11.4999V16.4999C12.75 16.9142 12.4142 17.2499 12 17.2499C11.5858 17.2499 11.25 16.9142 11.25 16.4999L11.25 11.4999C11.25 11.0857 11.5858 10.7499 12 10.7499Z'
        fill={color}
      />
      <path
        d='M12 8.99994C12.5523 8.99994 13 8.55223 13 7.99994C13 7.44766 12.5523 6.99994 12 6.99994C11.4477 6.99994 11 7.44766 11 7.99994C11 8.55223 11.4477 8.99994 12 8.99994Z'
        fill={color}
      />
      <path
        fillRule='evenodd'
        clipRule='evenodd'
        d='M7.31673 3.7687C10.4043 3.42362 13.5957 3.42362 16.6832 3.7687C18.5096 3.97282 19.9845 5.41146 20.1994 7.24843C20.5686 10.4053 20.5686 13.5945 20.1994 16.7515C19.9845 18.5884 18.5096 20.0271 16.6832 20.2312C13.5957 20.5763 10.4043 20.5763 7.31673 20.2312C5.49035 20.0271 4.01545 18.5884 3.8006 16.7515C3.43137 13.5945 3.43137 10.4053 3.8006 7.24843C4.01545 5.41146 5.49035 3.97282 7.31673 3.7687ZM16.5166 5.25942C13.5398 4.92671 10.4602 4.92671 7.48334 5.25942C6.33891 5.38732 5.42286 6.29057 5.29045 7.42268C4.93476 10.4638 4.93476 13.5361 5.29045 16.5772C5.42286 17.7093 6.33891 18.6126 7.48334 18.7405C10.4602 19.0732 13.5398 19.0732 16.5166 18.7405C17.6611 18.6126 18.5771 17.7093 18.7095 16.5772C19.0652 13.5361 19.0652 10.4638 18.7095 7.42268C18.5771 6.29057 17.6611 5.38732 16.5166 5.25942Z'
        fill={color}
      />
    </svg>
  );
};
